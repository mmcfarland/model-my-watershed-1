...
class ItsiService(OAuth2Service):
    ...
    initialized with site-wide parameters. It initializes the OAuth2Service with
    client_id, client_secret, and base_url taken from app settings, with
    AUTHORIZE_URL and ACCESS_TOKEN_URL which are explicitly defined, and with a
    custom ItsiSession object which is a sub-class of OAuth2Session and provides
    some convenience methods for getting data.
